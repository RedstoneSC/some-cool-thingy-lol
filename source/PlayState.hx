...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'PurpleVoid':
			{
				var bg:BGSprite = new BGSprite('Foig/PurpleVoid', -600, -200, 0.9, 0.9);
				add(bg);
				bg.active = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (((Note.charactersWith3D.contains(PlayState.SONG.player2)) ||  ((Note.charactersWith3D.contains(PlayState.SONG.player2)))))
            {
                for(i in 0...4) {
                    opponentStrums.members[i].texture = 'NOTE_assets_FOIG';
                }
		...
		if(ratingName == '?') {
			scoreTxt.text = 'NPS: ' + nps + ' | Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName;
		} else {
			scoreTxt.text = 'NPS: ' + nps + ' | Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
		...
		//EW STINKY OLD CODE I AHTE jk;dgrjkdgfjkmkjmdfgbmjkldfmlbjlmjkdfgbmjkmjkmjdflklmjldjmgkljmkdg
